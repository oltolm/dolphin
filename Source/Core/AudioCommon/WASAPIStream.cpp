...
static void ForEachNamedDevice(const std::function<bool(ComPtr<IMMDevice>, std::string)>& callback)
{
  ...
  Common::ScopeGuard result_guard([] { CoUninitialize(); });
  if (FAILED(result))
    result_guard.Dismiss();  // CoUninitialize must be matched with each successful CoInitialize
  // call, so don't call it if initialize fails
...
bool WASAPIStream::SetRunning(bool running)
{
  if (running)
  {
    ...
    Common::ScopeGuard need_data_event_guard([&] { CloseHandle(need_data_event); });
    if (need_data_event == NULL)
      need_data_event_guard.Dismiss();
